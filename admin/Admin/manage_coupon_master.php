...
if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	$res = mysqli_query($conn, "select * from coupon_master where id='$id'");
...
if (isset($_POST['submit'])) {
	...
	/*$res = mysqli_query($conn, "SELECT * from coupon_master where name='$coupon_code'");
	$check = mysqli_num_rows($res);
	if ($check > 0) {
		if (isset($_GET['id']) && $_GET['id'] != '') {
	...
	if ($msg == '') {
		if (isset($_GET['id']) && $_GET['id'] != '') {
			...
			mysqli_query($conn, "insert into coupon_master(coupon_code,coupon_value,coupon_type,cart_min_value,status) values('$coupon_code','$coupon_value','$coupon_type','$cart_min_value',1)");
		}
		header('location:coupon_master.php');
		die();
	}
